//===- all_gather_gemm_op_crossnode.cc ---------------------------- C++ ---===//
//
...
class AllGatherGemmOpCrossNode::AllGatherGemmOpCrossNodeImpl {
 private:
...
class AllGatherGemmOpCrossNode::AllGatherGemmOpCrossNodeImpl {
 public:
  AllGatherGemmOpCrossNodeImpl(
    ...
    FLUX_CHECK(false) << "compile with NVSHMEM to use AllGatherGemmOpCrossNode";
  }
  void
  reset_signals() {
    FLUX_CHECK(false) << "compile with NVSHMEM to use AllGatherGemmOpCrossNode";
...
AllGatherGemmOpCrossNode::AllGatherGemmOpCrossNode(
    std::shared_ptr<Group> tp_group,
    std::shared_ptr<Group> intra_node_group,
    int32_t nnodes,
    torch::Tensor output_buffer,
    ...
    : impl_(new AllGatherGemmOpCrossNodeImpl(
          tp_group,
          intra_node_group,
          nnodes,
          output_buffer,
          ...
          local_copy,
          ring_mode)) {}
AllGatherGemmOpCrossNode::~AllGatherGemmOpCrossNode() { delete impl_; }
void
AllGatherGemmOpCrossNode::reset_signals() {
  FLUX_CHECK(impl_ != nullptr) << "AllGatherGemmOpCrossNode not initialized";
  impl_->reset_signals();
}
void
AllGatherGemmOpCrossNode::copy_local(torch::Tensor input) {
  FLUX_CHECK(impl_ != nullptr) << "AllGatherGemmOpCrossNode not initialized";
  impl_->copy_local(input);
}
torch::Tensor
AllGatherGemmOpCrossNode::gemm_only(
    torch::Tensor input, torch::Tensor full_input, torch::Tensor weight) {
  FLUX_CHECK(impl_ != nullptr) << "AllGatherGemmOpCrossNode not initialized";
  return impl_->gemm_only(input, full_input, weight);
}
torch::Tensor
AllGatherGemmOpCrossNode::forward(torch::Tensor input, torch::Tensor weight) {
  FLUX_CHECK(impl_ != nullptr) << "AllGatherGemmOpCrossNode not initialized";
